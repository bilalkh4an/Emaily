...
import { 
  Inbox, Archive, Trash2, Send, Star, Edit2, 
  Settings, ChevronRight, ShieldCheck, CreditCard 
} from "lucide-react";
...
const SideNav = ({
  ...
  const getIcon = (folder, isActive) => {
    ...
    switch (folder) {
      ...
      case "Starred": return <Star {...props} />;
    ...
    <aside className="hidden lg:flex w-64 border-r border-gray-100 flex-col bg-white">
      ...
      <div className="px-6 py-7">
        <div className="flex items-center gap-3 px-1">
          ...
          <span className="font-black text-gray-800 tracking-tight text-lg">EmailShop</span>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 space-y-1 overflow-y-auto">
        <div className="px-3 mb-2">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.15em]">Mailboxes</p>
        </div>
          ...
          return (
            <button
              ...
              className={`group w-full flex items-center justify-between px-3 py-2.5 rounded-xl transition-all duration-200 ${
                isActive ? "bg-blue-50/80 text-blue-700" : "text-gray-500 hover:bg-gray-50 hover:text-gray-900"
              }`}
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className={`text-[14px] ${isActive ? "font-bold" : "font-semibold"}`}>{folder}</span>
              </div>
            </button>
          );
        })}
        ...
        <div className="pt-4 pb-2 px-3">
          ...
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.15em]">Preferences</p>
        </div>
        ...
        onClick={() => setIsSettingsOpen(true)}
          className={`group w-full flex items-center gap-3 px-3 py-2 rounded-xl transition-all duration-200 ${
            activeFolder === "Settings" ? "bg-blue-50/80 text-blue-700" : "text-gray-500 hover:bg-gray-50 hover:text-gray-900"
          }`}
        >
          <div className={`p-1.5 rounded-lg ${activeFolder === "Settings" ? "bg-white shadow-sm" : ""}`}>
            <Settings size={18} className={activeFolder === "Settings" ? "text-blue-600" : "text-gray-400 group-hover:rotate-45 transition-transform"} />
          </div>
          <span className={`text-[14px] ${activeFolder === "Settings" ? "font-bold" : "font-semibold"}`}>General Settings</span>
        </button>
      ...
      <div className="p-4 border-t border-gray-50 bg-gray-50/30">
        <div className="flex items-center justify-between mb-4 px-2">
            <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-blue-100 border-2 border-white shadow-sm flex items-center justify-center text-blue-600 font-bold text-xs">JD</div>
                <div className="min-w-0">
                    <p className="text-[12px] font-bold text-gray-800 truncate">John Doe</p>
